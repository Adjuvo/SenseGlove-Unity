...
/// <summary>A class to detect a SG_HandAnimator based on its SG_Feedback colliders </summary>
...
public class SG_HandDetector : MonoBehaviour 
{
    ...
    protected List<SG_SenseGloveHardware> detectedGloves = new List<SG_SenseGloveHardware>();
    ...
    private int HandModelIndex(SG_SenseGloveHardware model)
    {
        for (int i = 0; i < this.detectedGloves.Count; i++)
        {
            if (GameObject.ReferenceEquals(model.gameObject, this.detectedGloves[i].gameObject)) { return i; }
        }
        return -1;
    }
    ...
    /// <returns></returns>
    public SG_SenseGloveHardware[] GlovesInside()
    {
    ...
    /// <param name="model"></param>
    protected virtual void FireDetectEvent(SG_SenseGloveHardware model)
    {
    ...
    /// <summary> Fires when a new SG_GrabScript enters this detection zone and fullfils the detector's conditions. </summary>
    public event GloveDetectedEventHandler GloveDetected;

    protected void OnGloveDetected(SG_SenseGloveHardware model)
    {
    ...
    /// <summary>Fires when a SG_GrabScript exits this detection zone and fullfils the detector's conditions.  </summary>
    ...
    {
        int failsafe = this.detectedGloves.Count;
        int count = 0;
        while (count < failsafe && detectedGloves.Count > 0)
        {
            this.RemoveEntry(0);
            count++;
        }
...
public class GloveDetectionArgs : System.EventArgs
{
    ...
    public SG_SenseGloveHardware handModel;
