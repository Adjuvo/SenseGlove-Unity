...
namespace SG
{
    /// <summary> Type of SG_HandAnimator to debug hardware- and software models. </summary>
    public class SG_WireFrame : SG_HandAnimator
    {
        //------------------------------------------------------------------------------------------------------------------------------------
        // Properties
        ...
        private void SetupGlove(SG_SenseGloveData gloveData)
        {
        ...
        /// <param name="gloveData"></param>
        private void SetupFingers(SG_SenseGloveData gloveData)
        {
        ...
        protected override void SenseGlove_OnGloveLoaded(object source, EventArgs args)
        {
            ...
            SG_SenseGloveData data = this.senseGlove.GloveData;
        ...
        public override void UpdateHand(SG_SenseGloveData data)
        {
            if (data.dataLoaded)
            {
                //Update the glove
        ...
        public override void ResizeHand(float[][] newLengths)
        {
            ...
            {
                for (int f = 0; f < newLengths.Length && f < this.fingerJoints.Length; f++)
                {
                    for (int i = 0; i < newLengths[f].Length && i < this.fingerJoints[f].Length; i++)
                    {
        ...
        /// <param name="source"></param>
        /// <param name="args"></param>
        protected override void SenseGlove_OnCalibrationFinished(object source, SG_SenseGloveHardware.GloveCalibrationArgs args)
        {
