...
namespace SG
{
	...
	public class SG_CalibrationVoid : MonoBehaviour
	{
		...
		{
			if (hand != null && hand.gloveHardware != null) { hand.gloveHardware.checkCalibrationOnStart = checkOnStart; }
		}
		...
		public bool MovedEnough()
		{
			...
			if (leftCalibration != null && calibratingLeft)
			{
				...
				if (leftCalibration.CanAnimate) 
				{
					...
					{
						leftMoved = true;
						user.leftHand.gloveHardware.SendCmd(wf_movedEnough);
					}
					actual++; 
				}
			}
			if (rightCalibration != null && calibratingRight)
			{
				...
				{ 
                    ...
                    {
						rightMoved = true;
						user.rightHand.gloveHardware.SendCmd(wf_movedEnough);
                    }
					actual++; 
				}
			}
        ...
        {
			if (animater != null && calibrating) { animater.UpdateHand(pose); }
        }
		...
		protected int TryLinkingGloves()
        {
            ...
            if (user.LeftHandConnected)
            {
                ...
                {
                    user.leftHand.gloveHardware.SendCmd(this.wf_connected); //if user's hand is connected, hand & gloveHardware is not NULL
                }
            ...
            if (user.RightHandConnected)
            {
                ...
                {
                    user.rightHand.gloveHardware.SendCmd(this.wf_connected); //if user's hand is connected, hand & gloveHardware is not NULL
                }
					...
					if (calibratingRight && rightCalibration.CalibrationActive) 
					{
						if (rightConf)
						{
							...
							if (rightTUTime >= confirmTime)
							{
								...
								if (wf_Confirmed != null) { user.rightHand.gloveHardware.SendCmd(wf_Confirmed); }
							}
						}
						else { rightTUTime = 0; }
					}
					if (calibratingLeft && leftCalibration.CalibrationActive)
					{
						if (leftConf)
						{
							...
							if (leftTUTime >= confirmTime)
							{
								...
								if (wf_Confirmed != null) { user.leftHand.gloveHardware.SendCmd(wf_Confirmed); }
							}
						}
						else { leftTUTime = 0; }
					}
